...
| 写「原来那条判断为什么错」 | **不要把「通常成立」写成「必然 / 从未 / 唯一」。** 同一个病被 Codex 连续多轮抓到，原话有：「判据永远不可能通过」、「三段各自跑一次 CI」（R0 根本没有 CI）、「归因唯一」、「前提从未成立」、「两半都已经在类型里」、「fingerprint 是完整报告的子集」（哈希不是子集，payload 才是投影）。规则：写这类句子前先问「有没有反例」，有就加回限定词；**以及别用一个相关的量代替要陈述的那个量** —— 阶段数 ≠ 文件数、哈希 ≠ 子集、纯文档提交 ≠ 有 CI、单元序号在 manifest 上 ≠ 已在类型里。**改错时同样适用**：把「从未成立」改成「与问题无关」时，理由本身也得站得住（阶梯里真有 fingerprint 形态的级，所以「可能成立」成立）。 |
